...
func NewRemoteImageService(endpoint string, connectionTimeout time.Duration) (internalapi.ImageManagerService, error) {
	logrus.Infof("Connecting to image service %s", endpoint)
	...
	if err != nil {
		logrus.Errorf("Connect remote image service %s failed: %v", addr, err)
		return nil, err
	}
...
func (r *remoteImageService) ListImages(filter *runtimeapi.ImageFilter) ([]*runtimeapi.Image, error) {
	...
	if err != nil {
		logrus.Errorf("ListImages with filter %+v from image service failed: %v", filter, err)
		return nil, err
	}
...
func (r *remoteImageService) ImageStatus(image *runtimeapi.ImageSpec) (*runtimeapi.Image, error) {
	...
	if err != nil {
		logrus.Errorf("ImageStatus %q from image service failed: %v", image.Image, err)
		return nil, err
	}
	...
	if resp.Image != nil {
		if resp.Image.Id == "" || resp.Image.Size_ == 0 {
			...
			logrus.Errorf("ImageStatus failed: %s", errorMessage)
...
func (r *remoteImageService) PullImage(image *runtimeapi.ImageSpec, auth *runtimeapi.AuthConfig, podSandboxConfig *runtimeapi.PodSandboxConfig) (string, error) {
	...
	if err != nil {
		logrus.Errorf("PullImage %q from image service failed: %v", image.Image, err)
		return "", err
	}

	if resp.ImageRef == "" {
		errorMessage := fmt.Sprintf("imageRef of image %q is not set", image.Image)
		logrus.Errorf("PullImage failed: %s", errorMessage)
...
func (r *remoteImageService) RemoveImage(image *runtimeapi.ImageSpec) error {
	...
	if err != nil {
		logrus.Errorf("RemoveImage %q from image service failed: %v", image.Image, err)
		return err
	}
...
func (r *remoteImageService) ImageFsInfo() ([]*runtimeapi.FilesystemUsage, error) {
	...
	if err != nil {
		logrus.Errorf("ImageFsInfo from image service failed: %v", err)
		return nil, err
	}

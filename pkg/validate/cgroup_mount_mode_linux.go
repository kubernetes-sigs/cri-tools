...
var _ = framework.KubeDescribe("CgroupMountMode", func() {
	...
	Context("when the runtime advertises cgroup_mount_mode", func() {
		...
		It(
			"runtime should reject CGROUP_MOUNT_MODE_READ_ONLY for a privileged container [Conformance]",
			func(ctx SpecContext) {
				podID, podConfig = createPrivilegedPodSandbox(ctx, rc, true)

				By(
					"verifying the sandbox and image can create a privileged container with an unspecified mount mode",
				)
				...
				By("verifying container creation rejects the read-only request")

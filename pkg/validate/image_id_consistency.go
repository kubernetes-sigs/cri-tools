...
var _ = framework.KubeDescribe("Image Identifier Consistency", func() {
	...
	It(
		"should return the same image identifier across all APIs [Conformance]",
		Serial,
		func(ctx SpecContext) {
			...
			framework.Logf(
				"Verifying consistency: PullImageResponse.image_ref=%q, Image.id=%q, ContainerStatus.image_id=%q, Container.image_id=%q",
				imageRef,
				imageStatus.GetId(),
				containerStatus.GetImageId(),
				containers[0].GetImageId(),
			)

			// We use a custom failure message to show all mismatches at once
			Expect(
				imageStatus.GetId(),
			).To(Equal(imageRef), "Image.id MUST match PullImageResponse.image_ref")

			if containerStatus.GetImageId() == "" {
				Skip(
					"ContainerStatus.image_id is empty; runtime does not seem to implement CRI API image_id correctly yet",
				)
			}

			Expect(
				containerStatus.GetImageId(),
			).To(Equal(imageRef), "ContainerStatus.image_id MUST match PullImageResponse.image_ref")

			if containers[0].GetImageId() == "" {
				Skip(
					"Container.image_id is empty; runtime does not seem to implement CRI API image_id correctly yet",
				)
			}

			Expect(
				containers[0].GetImageId(),
			).To(Equal(imageRef), "Container.image_id MUST match PullImageResponse.image_ref")
		},
	)

	It(
		"should return the same image identifier when pulled from different registries [Conformance]",
		Serial,
		func(ctx SpecContext) {
			Expect(
				testSameImageDifferentRegistries,
			).To(HaveLen(2), "testSameImageDifferentRegistries should have 2 images")
			...
			Expect(
				imageRef1,
			).To(Equal(imageRef2), "Image IDs MUST be the same even if pulled from different registries if they refer to the same underlying image")
			...
			Expect(
				status1.GetId(),
			).To(Equal(imageRef1), "Image ID for registry 1 MUST match pull ref")
			...
			Expect(
				status2.GetId(),
			).To(Equal(imageRef1), "Image ID for registry 2 MUST match pull ref from registry 1")
			...
			Expect(
				containerStatus.GetImageId(),
			).To(Equal(imageRef1), "ContainerStatus.image_id MUST match the image ID from the first pull")
		},
	)
})

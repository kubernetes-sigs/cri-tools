...
func RegisterFlags() {
	...
	if runtime.GOOS == OSWindows {
		flag.BoolVar(
			...
			"lcow",
			false,
			"Run Linux container on Windows tests instead of Windows container tests",
		)
	} else {
	...
	flag.StringVar(
		&TestContext.NRISocketPath,
		"nri-socket",
		"",
		"Path to the NRI socket. NRI tests are skipped when not set.",
	)
}

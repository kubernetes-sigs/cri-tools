...
var _ = DescribeTable(
	...
	Entry(
		"returns filtered images with many reference --filter=reference=busybox, --filter=reference=k8s",
		[]*pb.Image{
			...
			fakeImage(
				"5",
				[]string{"registry.k8s.io/e2e-test-images/busybox@sha256:5"},
				[]string{"1.2.2"},
			),
		},
...
var _ = DescribeTable("filterImagesListByBefore", assert,
	Entry("returns filtered images with --filter=before=<image-name>[:<tag>]",
		[]*pb.Image{
			fakeImage(
				"1",
				[]string{"docker.io/library/server@sha256:1"},
				[]string{"docker.io/library/server:0.0.0"},
			),
			fakeImage(
				"2",
				[]string{"docker.io/library/busybox@sha256:2"},
				[]string{"docker.io/library/busybox:1.2.0"},
			),
			fakeImage(
				"3",
				[]string{"docker.io/library/nginx@sha256:3"},
				[]string{"docker.io/library/nginx:1.0.0"},
			),
			fakeImage(
				"4",
				[]string{"docker.io/library/app@sha256:4"},
				[]string{"docker.io/library/app:1.2.2"},
			),
		},
